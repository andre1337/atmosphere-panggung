...
<script src="js/jquery-1.12.4.js"></script>
  ...
  <!-- Accordion --> 
    ...
    function changeImage() {
        ...
        image.src = "http://localhost/webmonitoring/grafik/datasuhu.php?" + new Date().getTime();
    }
...
countdown();
    ...
    <img id="myImage" src="http://localhost/webmonitoring/grafik/datasuhu.php?" width="1200" height="500" />
...
if($jmldata>0){
	...
	if($page > 1){
		$prev=$page-1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$prev&mnu=suhu'>« Prev</a></span> ";
	}
	else{echo "<span class=disabled>« Prev</span> ";}

	// Tampilkan link page 1,2,3 ...
	for($i=1;$i<=$jmlhal;$i++)
	if ($i != $page){echo "<a href='$_SERVER[PHP_SELF]?page=$i&mnu=suhu'>$i</a> ";}
	else{echo " <span class=current>$i</span> ";}

	// Link kepage berikutnya (Next)
	if($page < $jmlhal){
		$next=$page+1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$next&mnu=suhu'>Next »</a></span>";
	}
