...
<script src="js/jquery-1.12.4.js"></script>
  ...
  <!-- Accordion --> 
    ...
    function changeImage() {
        ...
        image.src = "http://localhost/webmonitoring/grafik/datakelembapan.php?" + new Date().getTime();
    }
...
countdown();
    ...
    <img id="myImage" src="http://localhost/webmonitoring/grafik/datakelembapan.php?" width="1200" height="500" />
  ...
  $sqlc="select distinct(tgl) from `$tbmonitoring` order by `tgl` desc";
	$arrc=getData($conn,$sqlc);
		foreach($arrc as $dc) {							
				$tgl=$dc["tgl"];
				?>
  <h3>Data Kelembapan <?php echo $tgl;?></h3>
  <div>
<br />
<!-- Accordion -->
...
if($jmldata>0){
	...
	if($page > 1){
		$prev=$page-1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$prev&mnu=kelembapan'>« Prev</a></span> ";
	}
	else{echo "<span class=disabled>« Prev</span> ";}

	// Tampilkan link page 1,2,3 ...
	for($i=1;$i<=$jmlhal;$i++)
	if ($i != $page){echo "<a href='$_SERVER[PHP_SELF]?page=$i&mnu=kelembapan'>$i</a> ";}
	else{echo " <span class=current>$i</span> ";}

	// Link kepage berikutnya (Next)
	if($page < $jmlhal){
		$next=$page+1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$next&mnu=kelembapan'>Next »</a></span>";
	}
